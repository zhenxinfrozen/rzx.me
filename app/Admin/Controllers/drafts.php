...
/**
 * Drafts 管理控制器
 * 负责 Drafts 页面分组排序、图片管理及相关 AJAX 操作
 */
...
if ($ajaxAction) {
    handleDraftsAjax($ajaxAction, $configPath, $imagesRoot, $trashRoot, $imageOrderPath);
    exit;
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    handleDraftsFormSubmission($configPath, $imagesRoot, $trashRoot);
    header('Location: ' . ($_SERVER['REQUEST_URI'] ?? './drafts.php'));
    exit;
}
...
$page_title = '🛠️ Drafts 管理';
$page_subtitle = '管理 Drafts 页面分组与图片';
$_GET['page'] = 'drafts';
 ...
 * 处理表单提交
 */
function handleDraftsFormSubmission(string $configPath, string $imagesRoot, string $trashRoot): void
{
    try {
        ...
        if (saveDraftsConfig($configPath, $config)) {
            $_SESSION['drafts_flash'] = ['type' => 'success', 'text' => '配置保存成功！'];
        } else {
            throw new RuntimeException('配置保存失败，请检查文件权限');
        }
    } catch (Throwable $e) {
        $_SESSION['drafts_flash'] = ['type' => 'error', 'text' => '保存失败：' . $e->getMessage()];
    }
}
 ...
 * 处理 AJAX 请求
 */
function handleDraftsAjax(string $action, string $configPath, string $imagesRoot, string $trashRoot, string $imageOrderPath): void
{
    switch ($action) {
        case 'thumbnails':
...
function saveDraftsConfig(string $configPath, array $config): bool
{
...
function uploadCategoryThumbnail(string $imagesRoot): void
{
    ...
    $debugInfo['POST_category'] = $_POST['category'] ?? 'null';
    $debugInfo['FILES_present'] = isset($_FILES['thumbnail']) ? 'yes' : 'no';
    ...
    try {
        ...
        if (!isset($config['category_thumbnails'])) {
            $config['category_thumbnails'] = [];
            $debugInfo['created_category_thumbnails_key'] = true;
        }
        ...
        $debugInfo['verify_thumbnail_saved'] = isset($verifyConfig['category_thumbnails'][$category]) ? 'yes' : 'no';
        $debugInfo['verify_thumbnail_value'] = $verifyConfig['category_thumbnails'][$category] ?? 'not found';
...
function loadImageOrderConfig(string $imageOrderPath): array
{
    ...
    // 从新的合并结构中提取Drafts模块的数据
    if (isset($config['modules']['Drafts'])) {
        return $config['modules']['Drafts'];
    }
...
function saveImageOrderConfig(string $imageOrderPath, array $config): bool
{
    ...
    // 更新Drafts模块的配置
    $fullConfig['modules']['Drafts'] = $config;
