...
<form method="post" id="configForm">
    ...
    <input type="hidden" name="sort_method" value="<?= htmlspecialchars($currentConfig['sort_method'] ?? 'custom_order') ?>">
</form>
...
function deleteCurrentCategory() {
    if (!currentEditingCategory) return;
    if (!confirm(`确定要删除分组 \"${currentEditingCategory}\" 吗？`)) return;
...
function loadCategoryThumbnails(categoryName) {
    ...
    fetch(`${controllerUrl}?ajax=thumbnails&category=${encodeURIComponent(categoryName)}`)
        ...
        .then(data => {
            ...
            html += `<div class="add-image-btn" onclick="showImageUpload('${categoryName}')" title="添加图片"><i data-feather="plus"></i></div>`;
...
function showImageUpload(categoryName) {
    ...
    input.onchange = function(e) {
        if (e.target.files.length > 0) {
            uploadImages(categoryName, e.target.files);
        }
    };
    input.click();
}
...
function uploadImages(categoryName, files) {
    ...
    const oversized = Array.from(files).filter(file => file.size > maxSize);
...
function createNewCategory() {
    ...
    fetch(`${controllerUrl}?ajax=create_category`, {
        ...
        body: JSON.stringify({ category: categoryName, displayName, description, position })
    })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
