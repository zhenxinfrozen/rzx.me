...
## 0. 站点根目录
- 宝塔站点的运行目录请指向仓库中的 `public/`。
- 这样 `index.php`、静态资源和路由规则才能保持一致。
...
## 8. 路由模式
- 默认使用 `index.php?route=/...` 的查询参数模式，宝塔不配伪静态也能访问。
- 如果面板里已经配置了 Nginx/Apache 伪静态，再把 `app/Config/app.php` 里的 `use_pretty_urls` 改成 `true`。
